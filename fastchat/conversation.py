...
class Conversation:
    ...
    def get_prompt(self) -> str:
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            ret_sys = "" if system_prompt == "" else system_prompt + self.sep + "\n"
            ...
            if len(self.messages) >= 2 and self.messages[-2][0] == self.messages[-1][0] and not self.messages[-1][1]:
                role_last, message_last = self.messages[-2]
                ret_list[-2] = role_last + "\n" + message_last
                ret_list.pop()

            ret = ret_sys + "".join(ret_list)
            return ret
            ...
            for i, (role, message) in enumerate(self.messages):
                if message:
                    ...
                    ret += role + ":"
            return ret
        elif self.sep_style in [SeparatorStyle.QWEN_VL, SeparatorStyle.QWEN2_VL]:
            # [NOTE]: Qwen-VL / Qwen2-VL will return a blob of string and dict instead of a single string.

            def _adapt_format(msg):
                if not isinstance(msg, (list, tuple)):
                    return msg
                if self.sep_style == SeparatorStyle.QWEN_VL:
                    # Qwen-VL: to Qwen format
                    formatted_msg = []
                    for part in msg:
                        assert isinstance(part, dict)
                        if 'type' not in part:
                            formatted_msg.append(part)
                            continue
                        if part['type'] == 'text':
                            formatted_part = {'text': part['text']}
                        elif part['type'] == 'image_url':
                            formatted_part = {'image': part['image_url']['url']}
                        else:
                            assert False
                        formatted_msg.append(formatted_part)
                elif self.sep_style == SeparatorStyle.QWEN2_VL:
                    # Qwen2-VL: to OpenAI format
                    formatted_msg = []
                    for part in msg:
                        assert isinstance(part, dict)
                        if 'type' in part:
                            if 'image_url' in part:
                                image_url_content = part['image_url']
                                if not isinstance(image_url_content, str):
                                    assert isinstance(image_url_content, dict)
                                    assert 'url' in image_url_content
                                    part['image_url'] = image_url_content['url']
                            formatted_msg.append(part)
                            continue
                        formatted_part = part.copy()
                        for type_key in ['text', 'image', 'image_url', 'video']:
                            if type_key in part:
                                formatted_part['type'] = type_key
                        formatted_msg.append(formatted_part)
                else:
                    raise ValueError(f'Unexpected separator style {self.sep_style}')
                return formatted_msg

            messages = []
            for role, content in self.messages:
                messages.append((role, _adapt_format(content)))

            extra_response_prefix = None
            last_idx = len(messages) - 2
            if len(messages) >= 2 and messages[-2][0] == messages[-1][0] and not messages[-1][1]:
                extra_response_prefix = messages[-2][1]
                last_idx = len(messages) - 3
            query = messages[last_idx][1]

            assert last_idx % 2 == 0, 'history must be pairs of (user, assistant)'
            history = []
            for i in range(last_idx):
                ...
                if i % 2 == 0:
                    ...
                    assert role == self.roles[1]
                    response_i = message or ''
                    history.append((query_i, response_i))
            ...
            return json.dumps({
                ...
                'history': history,
                'query': query,
                'extra_response_prefix': extra_response_prefix,
            })
...
# Qwen-VL-Chat default template
# source: https://huggingface.co/Qwen/Qwen-VL-Chat/blob/main/qwen_generation_utils.py#L119
register_conv_template(
    Conversation(
        name="qwen-vl-chat",
        system_template="<|im_start|>system\n{system_message}",
        system_message="You are a helpful assistant.",
        roles=("<|im_start|>user", "<|im_start|>assistant"),
        sep_style=SeparatorStyle.QWEN_VL,
        sep="<|im_end|>",
        stop_token_ids=[
            151643,
            151644,
...
# Qwen2-VL-Chat default template
# source: TODO
register_conv_template(
    Conversation(
        name="qwen2-vl-chat",
        ...
        system_message="You are a helpful assistant.",
        roles=("<|im_start|>user", "<|im_start|>assistant"),
        sep_style=SeparatorStyle.QWEN2_VL,
        sep="<|im_end|>",
        stop_token_ids=[
            151643,
            151644,
...
if __name__ == "__main__":
    ...
    conv.append_message(conv.roles[1], "Hi!")
    conv.append_message(conv.roles[0], "How are you?")
    conv.append_message(conv.roles[1], "I am")
    conv.append_message(conv.roles[1], None)
    print(conv.get_prompt())
    ...
    print("-- Deepseek template without system --")
    conv = get_conv_template("deepseek-chat")
    conv.append_message(conv.roles[0], "QUERY-1")
    conv.append_message(conv.roles[1], "RESPONSE-1")
    conv.append_message(conv.roles[0], "QUERY-2")
    conv.append_message(conv.roles[1], "RESPONSE-2")
    conv.append_message(conv.roles[0], "QUERY-3")
    conv.append_message(conv.roles[1], None)
    print(conv.get_prompt())

    print("-- Deepseek template with system --")
    conv = get_conv_template("deepseek-chat")
    conv.set_system_message("SYSTEM-STRING")
    conv.append_message(conv.roles[0], "QUERY-1")
    conv.append_message(conv.roles[1], "RESPONSE-1")
    conv.append_message(conv.roles[0], "QUERY-2")
    conv.append_message(conv.roles[1], "RESPONSE-2")
    ...
    print("-- Qwen-VL template --")
    conv = get_conv_template("qwen-vl-chat")
    conv.append_message(conv.roles[0], "QUERY-1")
    conv.append_message(conv.roles[1], "RESPONSE-1")
    conv.append_message(conv.roles[0], "QUERY-2")
    conv.append_message(conv.roles[1], "RESPONSE-2")
    conv.append_message(conv.roles[0], "QUERY-3")
    conv.append_message(conv.roles[1], "EXTRA-RESPONSE-3")
    conv.append_message(conv.roles[1], None)
    print(conv.get_prompt())

    print("-- Qwen-VL template with image --")
    conv = get_conv_template("qwen-vl-chat")
    conv.append_message(conv.roles[0], "QUERY-1")
    conv.append_message(conv.roles[1], "RESPONSE-1")
    conv.append_message(conv.roles[0], [{"text": "QUERY-2"}, {"image": "IMAGE-2"}])
    conv.append_message(conv.roles[1], "RESPONSE-2")
    conv.append_message(conv.roles[0], "QUERY-3")
    conv.append_message(conv.roles[1], "EXTRA-RESPONSE-3")
    conv.append_message(conv.roles[1], None)
    ...
    print("-- Qwen-VL template with image (openai api) --")
    conv = get_conv_template("qwen-vl-chat")
    conv.append_message(conv.roles[0], "QUERY-1")
    conv.append_message(conv.roles[1], "RESPONSE-1")
    conv.append_message(conv.roles[0], [
        {"type": "text", "text": "QUERY-2"},
    ...
    print("-- Qwen2-VL template with image and pixel info --")
    conv = get_conv_template("qwen2-vl-chat")
    conv.append_message(conv.roles[0], "QUERY-1")
    conv.append_message(conv.roles[1], "RESPONSE-1")
    conv.append_message(conv.roles[0], [
        {"text": "QUERY-2"},
        {"image": "IMAGE-2", "type": "image", "min_pixels": 6, "max_pixels": 100}
    ])
    conv.append_message(conv.roles[1], "RESPONSE-2")
    conv.append_message(conv.roles[0], "QUERY-3")
    conv.append_message(conv.roles[1], "EXTRA-RESPONSE-3")
