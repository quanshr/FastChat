...
class KVCache:
    ...
    def copy(self, indices: torch.Tensor, prev_length: int, dim: int = 1):
        """
    ...
    def cat(self, tensor: torch.Tensor, dim: int = 1):
        """
...
def initialize_past_key_values(model):
    ...
    for i in range(config.num_hidden_layers):
       try:
           device = model.transformer.h[i].attn.c_proj.weight.device
       except:
           try:
...
if __name__ == '__main__':
    ...
    model = AutoModelForCausalLM.from_pretrained('/cpfs01/shared/public/feihu.hf/eval_ckpt/Qwen-7B-Chat/', trust_remote_code=True)
    past_key_values, past_key_values_data_list, current_length_data = initialize_past_key_values(model)
    # print(past_key_values[0][0].shape)
    from typing import List, Optional, Tuple
    def func(cache: Tuple[Tuple[KVCache]]):
        print(isinstance(cache[0][0], torch.Tensor))
        print('hello')
        print(cache[0][0].shape)
    func(past_key_values)