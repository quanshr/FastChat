...
def initialize_tree(input_ids, model, tree_attn_mask, past_key_values, logits_processor):
    ...
    try:
        ...
        #     layer.attn.tree_mask = tree_attn_mask
...
def update_inference_inputs(
    ...
    for past_key_values_data in past_key_values_data_list:
        ...
        dst = past_key_values_data[..., prev_input_len: prev_input_len + tgt.shape[-3],:, :]
        # Copy relevant past information from the source to the destination
        dst.copy_(tgt, non_blocking=True)

    # Update the current length tensor (currently only support batch size is 1)
    current_length_data.fill_(prev_input_len + tgt.shape[-3])
