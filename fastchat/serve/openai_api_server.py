...
def check_requests(request) -> Optional[JSONResponse]:
    ...
    if request.min_p is not None and request.min_p > 1:
        return create_error_response(
            ErrorCode.PARAM_OUT_OF_RANGE,
            f"{request.min_p} is greater than the maximum of 1 - 'min_p'",
        )
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    if request.logprobs is None or request.logprobs is False:
        logprobs = None
    else:
        logprobs = request.top_logprobs
    if request.prompt_logprobs is None or request.prompt_logprobs is False:
    ...
    gen_params = await get_gen_params(
        ...
        conv_template=request.conv_template,
    )
    ...
    for i, content in enumerate(all_tasks):
        ...
        logprobs_dicts = content.get('logprobs', None)
        logprobs = None if not logprobs_dicts else ChatCompletionLogProbs.model_validate(logprobs_dicts[0])
        prompt_logprobs_dicts = content.get('prompt_logprobs', None)
        prompt_logprobs = None if not prompt_logprobs_dicts else ChatCompletionLogProbs.model_validate(
            prompt_logprobs_dicts[0])
...
async def chat_completion_stream_generator(
    ...
    for i in range(n):
        ...
        async for content in generate_completion_stream(gen_params, worker_addr):
            ...
            # FIXME (suyang.fy): We do not change '' to None for bailian API.
            # if len(delta_text) == 0:
            #     delta_text = None
            logprobs_dicts = content.get('logprobs', None)
            logprobs = None if not logprobs_dicts else ChatCompletionLogProbs.model_validate(logprobs_dicts[0])
...
def create_openai_api_server():
    ...
    parser.add_argument("--log-level", type=str, default="info", help="App log level.")
